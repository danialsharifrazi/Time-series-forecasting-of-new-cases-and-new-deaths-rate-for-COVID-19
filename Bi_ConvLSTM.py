def RUN(block):
	...
	from keras.layers import ConvLSTM2D,Flatten,Dense,Bidirectional
	import numpy as np
	...
	model = Sequential()
	model.add(Bidirectional(ConvLSTM2D(filters=64, kernel_size=(1,2), activation='relu',return_sequences=True, input_shape=(n_seq, 1, n_steps_in, n_features))))
	model.add(Bidirectional(ConvLSTM2D(filters=64, kernel_size=(1,2), activation='relu',return_sequences=True)))
	model.add(Bidirectional(ConvLSTM2D(filters=64, kernel_size=(1,2),activation='relu')))
	model.add(Flatten())
	...
	#NetPlot(net)
	model.save(f'./results/BiConvLSTM_{block}_days')
	...
	actuals_path=f'./results/actuals_{block}_days_Bi-ConvLSTM.txt'
	predicteds_path=f'./results/predicteds_{block}_days_Bi-ConvLSTM.txt'
	metrics_path=f'./results/metrics_{block}_days_Bi-ConvLSTM.txt'
	...
	plt.figure(f'Forecasting {block} days_plot_Bi-ConvLSTM',dpi=200)
	plt.plot(actuals,color='black')
	plt.plot(predicteds,color='green')
	plt.xlabel('Day')
	plt.ylabel('New Cases')
	plt.legend(['Actuals','Predicts'])
	plt.savefig(f'./results/Forecasting {block} days_plot_Bi-ConvLSTM')


	plt.figure(f'Forecasting {block} days_scatter_Bi-ConvLSTM',dpi=200)
	plt.plot(actuals,actuals,color='red')
	plt.plot(actuals,predicteds,'bo',color='green')
	plt.xlabel('Observation')
	plt.ylabel('Predict')
	plt.legend(['Actuals','Predicts'])
	plt.savefig(f'./results/Forecasting {block} days_scatter_Bi-ConvLSTM')
