def RUN(block):	
	...
	model.save(f'./results/BiGRU_{block}_days')
	...
	actuals_path=f'./results/actuals_{block}_days_Bi-GRU.txt'
	predicteds_path=f'./results/predicteds_{block}_days_Bi-GRU.txt'
	metrics_path=f'./results/metrics_{block}_days_Bi-GRU.txt'
	...
	plt.figure(f'Forecasting {block} days_plot_Bi-GRU',dpi=200)
	plt.plot(actuals,color='black')
	plt.plot(predicteds,color='green')
	plt.xlabel('Day')
	plt.ylabel('New Cases')
	plt.legend(['Actuals','Predicts'])
	plt.savefig(f'./results/Forecasting {block} days_plot_Bi-GRU')

	plt.figure(f'Forecasting {block} days_scatter_Bi-GRU',dpi=200)
	plt.plot(actuals,actuals,color='red')
	plt.plot(actuals,predicteds,'bo',color='green')
	plt.xlabel('Observation')
	plt.ylabel('Predict')
	plt.legend(['Actuals','Predicts'])
	plt.savefig(f'./results/Forecasting {block} days_scatter_Bi-GRU')
