def RUN(block):	
	...
	model = Sequential()
	model.add(Conv1D(64,2, activation='relu',padding='same', input_shape=(n_steps_in, n_features)))
	...
	actuals_path=f'./results/actuals_{block}_days_Proposed.txt'
	predicteds_path=f'./results/predicteds_{block}_days_Proposed.txt'
	metrics_path=f'./results/metrics_{block}_days_Proposed.txt'
	...
	plt.figure(f'Forecasting {block} days_plot_Proposed',dpi=200)
	plt.plot(actuals,color='black')
	plt.plot(predicteds,color='green')
	plt.xlabel('Day')
	plt.ylabel('New Cases')
	plt.legend(['Actuals','Predicts'])
	plt.savefig(f'./results/Forecasting {block} days_plot_Proposed')


	plt.figure(f'Forecasting {block} days_scatter_Proposed',dpi=200)
	plt.plot(actuals,actuals,color='red')
	plt.plot(actuals,predicteds,'bo',color='green')
	plt.xlabel('Observation')
	plt.ylabel('Predict')
	plt.legend(['Actuals','Predicts'])
	plt.savefig(f'./results/Forecasting {block} days_scatter_Proposed')
